...
pub struct ToolAdapter {
    ...
    /// Read-only discovery roots holding vendor/plugin-managed skills (e.g.
    /// Codex's `~/.codex/plugins/cache`). Skills found here are displayed but
    /// must never be deleted/synced/imported — the vendor overwrites them on
    /// upgrade. Scanned recursively (deeply-nested bundle layouts).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub readonly_scan_dirs: Vec<String>,
...
impl ToolAdapter {
    ...
    /// Returns the existing read-only discovery roots for this adapter (vendor/
    /// plugin-managed skill dirs). Skills under these are display-only.
    pub fn readonly_existing_scan_dirs(&self) -> Vec<PathBuf> {
        let mut dirs = Vec::new();
        for rel in &self.readonly_scan_dirs {
            for c in Self::candidate_paths(rel) {
                if c.exists() && !dirs.contains(&c) {
                    dirs.push(c);
                }
            }
...
pub fn default_tool_adapters() -> Vec<ToolAdapter> {
    vec![
        ...
        ToolAdapter {
            ...
            // Installed plugins are unpacked under `~/.claude/plugins/cache/`
            // in a nested `<marketplace>/<plugin>/<version>/skills/<skill>`
            // bundle layout (e.g. the official `superpowers` marketplace).
            // These are vendor-managed — Claude overwrites them on plugin
            // upgrade — so they are scanned read-only (display-only, never
            // synced/imported/deleted), mirroring Codex's plugin cache.
            key: "claude_code".into(),
            display_name: "Claude Code".into(),
            relative_skills_dir: ".claude/skills".into(),
            relative_detect_dir: ".claude".into(),
            additional_scan_dirs: vec![],
            readonly_scan_dirs: vec![".claude/plugins/cache".into()],
            override_skills_dir: None,
            category: ToolCategory::Coding,
            is_custom: false,
            recursive_scan: false,
        ...
        ToolAdapter {
            ...
            // actually reads.
            //
            // Codex also ships built-in system skills under
            // `~/.codex/skills/.system/` (imagegen, openai-docs, plugin-creator,
            // review-agent, skill-creator, skill-installer). Those are
            // vendor-managed and regenerated by Codex, so they are scanned
            // read-only — display-only, never synced/imported/deleted. The
            // primary scan skips dot-dirs, so `.system` only surfaces via the
            // read-only root.
            key: "codex".into(),
        ...
        ToolAdapter {
            ...
            //
            // Grok ships built-in skills under `~/.grok/bundled/skills/` and
            // unpacks installed marketplace plugins under
            // `~/.grok/marketplace-cache/<hash>/.../skills/<skill>`. Both are
            // vendor-managed (regenerated by Grok), so they are scanned
            // read-only — display-only, never synced/imported/deleted.
            key: "grok".into(),
            display_name: "Grok".into(),
            relative_skills_dir: ".grok/skills".into(),
            relative_detect_dir: ".grok".into(),
            additional_scan_dirs: vec![],
            readonly_scan_dirs: vec![".grok/bundled/skills".into(), ".grok/marketplace-cache".into()],
            override_skills_dir: None,
            category: ToolCategory::Coding,
            is_custom: false,
            recursive_scan: false,
        ...
        ToolAdapter {
            ...
            // in the TRAE tab too. TRAE ships built-in system skills under
            // `~/.trae/skills/.system/` (plugin-creator, skill-creator,
            // skill-installer); those are vendor-managed and regenerated by
            // TRAE, so they are scanned read-only — display-only, never
            // synced/imported/deleted. The primary scan skips dot-dirs, so
            // `.system` only surfaces via this read-only root.
            key: "trae".into(),
            display_name: "TRAE IDE".into(),
            relative_skills_dir: ".trae/skills".into(),
            relative_detect_dir: ".trae".into(),
            additional_scan_dirs: vec![".agents/skills".into()],
            readonly_scan_dirs: vec![".trae/skills/.system".into()],
            override_skills_dir: None,
            category: ToolCategory::Coding,
            is_custom: false,
            recursive_scan: false,
        ...
        ToolAdapter {
            ...
            // shared cross-client `~/.agents/skills/` discovery location, and
            // vendor-managed system skills under `~/.trae-cn/skills/.system/`
            // scanned read-only. (May not exist on every install;
            // readonly_existing_scan_dirs filters by existence.)
            key: "trae_cn".into(),
            display_name: "TRAE CN".into(),
            relative_skills_dir: ".trae-cn/skills".into(),
            relative_detect_dir: ".trae-cn".into(),
            additional_scan_dirs: vec![".agents/skills".into()],
            readonly_scan_dirs: vec![".trae-cn/skills/.system".into()],
            override_skills_dir: None,
            category: ToolCategory::Coding,
            is_custom: false,
            recursive_scan: false,
...
mod tests {
    ...
    #[test]
    fn claude_code_scans_plugin_cache_read_only_only() {
        let adapter = default_tool_adapters()
            .into_iter()
            .find(|adapter| adapter.key == "claude_code")
            .expect("claude_code adapter should exist");

        // The plugin cache holds vendor-managed skills: discoverable but
        // read-only, never a writable/deployable location.
        assert!(adapter.additional_scan_dirs.is_empty());
        assert_eq!(adapter.readonly_scan_dirs, vec![".claude/plugins/cache"]);
    }

    #[test]
    fn codex_scans_plugin_cache_and_system_skills_read_only() {
        let adapter = default_tool_adapters()
            .into_iter()
            .find(|adapter| adapter.key == "codex")
            .expect("codex adapter should exist");

        // Codex reads the shared .agents/skills for discovery, and treats both
        // its plugin cache and its built-in `.system` skills as read-only.
        assert_eq!(adapter.additional_scan_dirs, vec![".agents/skills"]);
        assert_eq!(
            adapter.readonly_scan_dirs,
            vec![".codex/plugins/cache", ".codex/skills/.system"]
        );
    }

    #[test]
    fn grok_scans_bundled_and_marketplace_dirs_read_only() {
        let adapter = default_tool_adapters()
            .into_iter()
            .find(|adapter| adapter.key == "grok")
            .expect("grok adapter should exist");
    ...
    fn trae_adapters_scan_shared_agents_skills_dir() {
        ...
        for key in ["trae", "trae_cn"] {
            ...
            // TRAE ships vendor-managed system skills under `<primary>/.system`,
            // scanned read-only so they are displayed but never mutated.
            assert!(
                adapter
                    .readonly_scan_dirs
                    .iter()
                    .any(|d| d.ends_with("/skills/.system")),
                "{key} should scan its .system dir read-only, got: {:?}",
                adapter.readonly_scan_dirs
            );
        }
    }
